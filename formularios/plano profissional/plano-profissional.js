/* ==========================================================================
   DEV RUBY — Briefing PROFISSIONAL
   plano-profissional.js
   --------------------------------------------------------------------------
...
(function () {
  ...
  var form = document.getElementById("form-plano-profissional");
  if (!form) return;

  var steps = Array.prototype.slice.call(form.querySelectorAll(".step"));
  var TOTAL = steps.length; // 10
  ...
  var FIELD_LABELS = {
    ...
    objetivos: "Objetivos",
    usuarios: "Usuários com acesso",
    qtd_usuarios: "Qtd. de usuários",
    permissoes: "Níveis de permissão",
  ...
  function requiredMessage(control) {
    switch (control.name) {
      ...
      case "descricao":
        return "Conte um pouco sobre o sistema para continuar.";
      default:
  ...
  function buildWhatsappLink() {
    ...
    linhas.push("*Briefing — Projeto PROFISSIONAL*");
    linhas.push("");
    ...
    var tipo = all("tipo_sistema");
    if (tipo.length) {
      linhas.push("*Tipo de sistema:* " + tipo.join(", "));
    }
    ...
    var usuarios = all("usuarios");
    if (usuarios.length) {
      linhas.push("*Usuários:* " + usuarios.join(", "));
    }
    if (v("qtd_usuarios")) linhas.push("*Qtd. usuários:* " + v("qtd_usuarios"));
    if (v("permissoes"))   linhas.push("*Níveis de permissão:* " + v("permissoes"));

    var funcionalidades = all("funcionalidades");
    if (funcionalidades.length) {
      linhas.push("*Funcionalidades:* " + funcionalidades.join(", "));
    }
    ...
    if (v("identidade")) linhas.push("*Identidade visual:* " + v("identidade"));

    var estilo = all("estilo");
    if (estilo.length) {
      linhas.push("*Estilo:* " + estilo.join(", "));
    }
    if (v("cores")) linhas.push("*Cores:* " + v("cores"));

    if (v("descricao")) {
      linhas.push("");
      linhas.push("*Sobre o sistema:*");
